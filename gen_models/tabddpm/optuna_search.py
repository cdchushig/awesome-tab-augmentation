...
def objective(trial, dataname, best_params=None):
    ...
    # Paths to scripts
    main_script = os.path.join("gen_models", "tabddpm", "main_train.py")
    sample_script = os.path.join("gen_models", "tabddpm", "main_sample.py")
...
def main(args):
    ...
    optuna_results_dataset_path = os.path.join(current_dir, "optuna_results", f"tabddpm_{args.dataname}")
    os.makedirs(optuna_results_dataset_path, exist_ok=True)
    
    study = optuna.create_study(direction="maximize", study_name=f"tabddpm_{args.dataname}", storage=f"sqlite:///{optuna_results_dataset_path}/{date_str}.db", load_if_exists=True)
