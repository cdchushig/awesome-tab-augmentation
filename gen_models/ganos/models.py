...
class BaseGAN():
    def __init__(self,
        ...
        if self.prefix is None:
            ...
            actual_path = Path(__file__).resolve().parents[0]
            self.prefix = f'{actual_path}/Experiments/results/{prefix}'
            print(f'No prefix provided. Using default: {self.prefix}')
        if self.write_to_disk:
            ...
            print(f'Creating folder for models/data samples/metrics/plots: {self.prefix}')
    ...
    def fit(self,
        ...
        writer = SummaryWriter(log_dir="gen_models/ganos/logs")
        ...
        X_tens = torch.Tensor(X.values) if isinstance(X, pd.DataFrame) else torch.Tensor(X)
        y_tens = torch.Tensor(y).view(-1, 1) if y is not None else None

        if y_tens is not None:
            print(f"y_tens shape: {y_tens.shape}")
        # Determine conditional mode based on netG, netD, and provided 'condition' parameter
        ...
        print(f"Conditional mode: {self.condition}")

        # Adjust input for non-conditional mode
        y_train = None
        if not self.condition:
            print("Adjusting X_tens and y_tens for non-conditional mode...")
            ...
            y_train = y
            print(f"New X_tens shape: {X_tens.shape}, updated cat_dims: {self.cat_dims}")

        # Initialize networks if they don't already exist
        ...
        # Initialize auxiliary classifier if necessary
        ...
        # Initialize auxiliary teacher if necessary
            ...
            if self.write_to_disk:
                ...
                if self.total_iters % self.save_data_every == 0 and self.total_iters > 0:
                    if self.condition:
                        ...
                        self._save_data(self.sample(n=25000))
            self.total_iters += 1
    ...
    def resample(self, X, y):
        ...
        if self.netG.condition:
            ...
            print(f"Needed minority cases: {n_minority_needed}, Generated minority cases: {len(y_fake)}")
...
class Generator(nn.Module):
    def __init__(self,
                 ...
                 condition_num_on_cat: bool = False,
                 # TODO implement
