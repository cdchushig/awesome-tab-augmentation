...
import os
import time
from gen_models.ganos.models import WGANGP
from utils_train import preprocess, TabularDataset, compute_difference_samples, balance_dataset
from gen_models.ganos.helpers import get_cat_dims  # Adjust path if necessary
...
def main(args):
    ...
    if args.model_dir is None:
        model_dir = f"gen_models/ganos/{args.dataname}/models"
        print(f"Model directory not provided. Using default path: {model_dir}")
    else:
    ...
    gan.netG = torch.load(f'{model_dir}/netG/netG_final.statedict')
    gan.netD = torch.load(f'{model_dir}/netD/netD_final.statedict')
    ...
    num_encoder = prep.named_transformers_['num'].named_steps['minmaxscaler']
    
    X_syn_cat = X_synthetic[:, len(num_cols):]    
    X_syn_num = X_synthetic[:, :len(num_cols)]

    X_syn_cat_orig = fitted_encoder.inverse_transform(X_syn_cat)
    X_syn_num_orig = num_encoder.inverse_transform(X_syn_num)

    X_synthetic_original = np.hstack([X_syn_num_orig, X_syn_cat_orig])
